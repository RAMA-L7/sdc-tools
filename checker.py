...
def check_sdc(text: str) -> CheckResult:
    ...
    for td in timing_derate:
        ...
        m = re.search(r'-early\s+-cell_delay\s+([\d.]+)', td)
        ...
        m = re.search(r'-late\s+-cell_delay\s+([\d.]+)', td)
        if m and float(m.group(1)) > 1.0:
        ...
        m = re.search(r'-early\s+-net_delay\s+([\d.]+)', td)
        if m and float(m.group(1)) < 1.0:
        ...
        m = re.search(r'-late\s+-net_delay\s+([\d.]+)', td)
        if m and float(m.group(1)) > 1.0:
    ...
    # Clock uncertainty hold vs setup ratio
    for u in clk_uncertainty:
        h_m = re.search(r'-hold\s+([\d.]+)', u)
        if h_m:
            h_val = float(h_m.group(1))
            s_m = re.search(r'-setup\s+([\d.]+)', u)
            if s_m:
                s_val = float(s_m.group(1))
                if s_val > 0 and abs(h_val / s_val - 0.5) > 0.15:
    ...
    try:
        ...
        for m in rel_result.mismatches:
            if m.severity == "warning":
                ...
                info.append(InfoItem(m.code, m.msg))
    ...
    if oper_cond:
        ...
        comment_lines = [l.strip() for l in text.splitlines() if l.strip().startswith('#')]
